...
sshpass -p $REMOTE_PASSWORD ssh -p $REMOTE_SSH_PORT $REMOTE_USERNAME@$REMOTE_HOST -o StrictHostKeyChecking=no << EOF
	...
	docker run -d --net host --name $IMAGE_REPOSITORY --restart always -v /var/data/nginx/conf.d/:/etc/nginx/conf.d/ /var/data/certbot/conf:/etc/letsencrypt -v /var/data/certbot/www:/var/www/certbot $IMAGE_REPOSITORY
	expect -c "
		set timeout 5
